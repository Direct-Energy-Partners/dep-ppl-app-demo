...
class WinlineCharger(Device):
    ...
    @property
    def gun_a_status(self) -> str | None:
        return self.read("measure.gunA.status")

    @property
    def gun_b_status(self) -> str | None:
        return self.read("measure.gunB.status")

    @property
    def is_charging(self) -> bool:
        return self.gun_a_status == "Charging" or self.gun_b_status == "Charging"
    ...
    def set_total_power(self, power_w: float) -> None:
        ...
        per_gun = min(power_w, self.power_max_w) / 2.0
